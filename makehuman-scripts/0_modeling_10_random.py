...
class RandomizeAction(gui3d.Action):
    def __init__(self, human, before, after):
        super(RandomizeAction, self).__init__("Randomize Custom")
        self.human = human
        self.before = before
        self.after = after
...
def randomize(human, symmetry, macro, height, face, body, sigmas): #[0.2,0.15,0.2,0.1,0.05]
    modifierGroups = []
    if macro:
    ...
    for m in modifiers:
        if m.fullName not in randomValues:
            ...
            elif m.groupName in ["ears", "eyebrows"]:
                sigma = sigmas[2]
            elif m.groupName in ["nose"]:
                sigma = sigmas[3]
            elif m.groupName in ["eyes", "cheek", "mouth"]:
                sigma = sigmas[0]
            elif m.groupName in ["forehead", "neck", "chin"]:
                sigma = sigmas[4]
